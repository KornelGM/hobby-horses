...
public class LeaderboardUI : MonoBehaviour, IServiceLocatorComponent, IStartable, IAwake
{
	...
	public void CustomStart()
	{
		...
		_leaderboardManager.MyFactory.OnMyFactoryPointsChanged += Refresh;

		SetUpFactories(_currentLeague.Factories);
		Refresh();
	}
	...
	public void SetUpFactories(List<Factory> factories)
	{
		if (factories == null) return;
		int loopEnd = Mathf.Min(factories.Count, _leaderboardPanels.Count);
		int i = 0;

		for (; i < loopEnd; i++)
		{
			_leaderboardPanels[i].Initialize(factories[i]);
		}
	}
	...
	public void Refresh()
	{
		...
		if (_currentLeague.Factories.Count == 20)
	...
	private void SortLeaderboardByPoints()
	{
		_leaderboardPanels = _leaderboardPanels
			.OrderByDescending(panel => panel.CurrentFactory == null ? 0 : panel.CurrentFactory.Points)
			.ThenByDescending(panel => panel.CurrentFactory != null && panel.CurrentFactory.IsMyFactory)
			.ToList();
		...
		for (int i = 0; i < _leaderboardPanels.Count; i++)
		{
			...
			if(_leaderboardPanels[i].CurrentFactory == null) return;
			_leaderboardPanels[i].CurrentFactory.Place = _leaderboardPanels[i].Place;
		}
	}
