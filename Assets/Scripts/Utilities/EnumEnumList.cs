...
[Serializable]
public class EnumEnumList<T1, T2> : IEnumerable where T1 : Enum where T2 : Enum
{
    public EnumEnumPair<T1, T2> this[int index] => _list[index];
    public int Count => _list.Count;

    [field: SerializeField] private T2 DefaultValue { get; set; }
    [SerializeField] private List<EnumEnumPair<T1, T2>> _list = new();
    
    public EnumEnumList(T2 defaultValue = default)
    {
        DefaultValue = defaultValue;
    }
    
    public EnumEnumList(List<T1> keys, T2 defaultValue = default)
    {
        DefaultValue = defaultValue;
        
        foreach (T1 key in keys)
        {
            _list.Add(new EnumEnumPair<T1, T2>(key, defaultValue));
        }
    }
    ...
    {
        _list.Add(new EnumEnumPair<T1, T2>(key, value));
    }
