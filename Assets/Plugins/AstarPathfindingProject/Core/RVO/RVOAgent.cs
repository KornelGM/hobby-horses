...
namespace Pathfinding.RVO.Sampled {
	...
	public class Agent : IAgent {
		...
		internal float radius, height, desiredSpeed, maxSpeed, agentTimeHorizon, obstacleTimeHorizon, agentTimeHorizonSqr;
		...
		public List<IAgent> PossibleCollisionNeighbours { get; } = new List<IAgent>();
		...
		public void BufferSwitch () {
			...
			agentTimeHorizonSqr = agentTimeHorizon * agentTimeHorizon;
		...
		internal struct VO {
			...
			public VO (Vector2 center, Vector2 offset, float radius, float inverseDt, float inverseDeltaTime) {
				...
				float magnitude = center.magnitude;
				// Collision?
				if (magnitude < radius) {
					colliding = true;
					...
					line1 = center.normalized * (magnitude - radius - 0.001f) * 0.3f * inverseDeltaTime;
			...
			public static VO SegmentObstacle (Vector2 segmentStart, Vector2 segmentEnd, Vector2 offset, float radius, float inverseDt, float inverseDeltaTime) {
				...
				float closestOnSegmentMagnitude = closestOnSegment.magnitude;
				// Collision?
				if (closestOnSegmentMagnitude <= radius) {
					vo.colliding = true;

					vo.line1 = closestOnSegment.normalized * (closestOnSegmentMagnitude - radius) * 0.3f * inverseDeltaTime;
		...
		void GenerateNeighbourAgentVOs (VOBuffer vos) {
			...
			PossibleCollisionNeighbours.Clear();
			...
			for (int o = 0; o < neighbours.Count; ++o) {
				...
				if ((position - other.position).sqrMagnitude > agentTimeHorizonSqr * CalculatedSpeed * CalculatedSpeed)
                {
					continue;
                }
				...
				if (other.locked)
				{
					PossibleCollisionNeighbours.Add(other);
				}
