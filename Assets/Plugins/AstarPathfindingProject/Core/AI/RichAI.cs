...
namespace Pathfinding {
	...
	public partial class RichAI : AIBase, IAstarAI {
		...
		public bool adjustPathIfAgentOnDestination = true;
		...
		public bool reachedDestination {
			get {
				...
				if (approachingPathEndpoint && distanceToSteeringTarget + movementPlane.ToPlane(destination - richPath.Endpoint).sqrMagnitude > endReachedDistance * endReachedDistance) return false;
		...
		void FinalMovement (Vector3 position3D, float deltaTime, float distanceToEndOfPath, float slowdownFactor, out Vector3 nextPosition, out Quaternion nextRotation) {
			...
			if (rvoController != null && rvoController.enabled) {
				...
				if (adjustPathIfAgentOnDestination && rvoController.rvoAgent.PossibleCollisionNeighbours.Count > 0)
				{
					Vector2 destination2D = new Vector2(destination.x, destination.z);
					foreach (IAgent agent in rvoController.rvoAgent.PossibleCollisionNeighbours)
					{
						float radiusSum = agent.Radius + rvoController.rvoAgent.Radius;
						if (radiusSum * radiusSum > (destination2D - agent.Position).sqrMagnitude)
						{
							Vector3 exitVector = destination - new Vector3(agent.Position.x, 0f, agent.Position.y);
							exitVector.y = 0f;
							exitVector.Normalize();

							if (exitVector == Vector3.zero)
                            {
								exitVector = (Vector3)seeker.GetCurrentPath().path[^2].position - (Vector3)seeker.GetCurrentPath().path[^1].position;
								exitVector.y = 0;
								exitVector.Normalize();
                            }

							Vector3 newDestination = new Vector3(agent.Position.x, destination.y, agent.Position.y) + radiusSum * LockedAgentPush * exitVector;
							destination = AstarPath.active.GetNearest(newDestination, seeker.GetCurrentPath().nnConstraint).position;
						}
					}
				}
			}
